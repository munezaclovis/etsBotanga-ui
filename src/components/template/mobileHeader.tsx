...
import { SET_MOBILE_HEADER, SET_MOBILE_SIDEBAR } from "../../store/theme/actions";
...
const MobileHeader = () => {
    ...
    return (
        ...
        >
            <Link to={"/"} className="font-medium text-lg uppercase flex flex-row items-center">
                Botanga
            </Link>
            <div className="flex flex-row items-center space-x-4">
                <button
                    className={`cursor-pointer outline-none align-middle${
                        theme.mobileSidebarOpen ? " text-slate-100 lg:text-slate-400" : " text-slate-400"
                    }`}
                    onClick={() => toggleSidebar()}
                >
                    <FontAwesomeIcon icon={"face-smile-tongue"} className="text-2xl font-light" />
                </button>
                <button
                    className={`cursor-pointer hover:text-slate-100${
                        theme.mobileHeaderOpen ? " text-slate-100 lg:text-slate-400" : " text-slate-400"
                    }`}
